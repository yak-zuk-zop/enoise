...
-spec hex_str_to_bin(string()) -> binary().
...
-spec blake2b_data() -> [#{input := binary(), output := binary()}].
blake2b_data() ->
    [#{ input => <<>>,
        output => hex_str_to_bin(
...
-spec blake2s_data() -> #{input := binary(), output := binary()}.
blake2s_data() ->
    #{ input => <<"abc">>,
       output => hex_str_to_bin(
...
-spec blake2b_hmac_data() -> [#{key := binary(), data := binary(), hmac := binary()}].
blake2b_hmac_data() ->
...
-spec noise_test_vectors() -> [map()].
noise_test_vectors() ->
    noise_test_filter(parse_test_vectors("test/test_vectors.txt")).
...
-spec noise_test_filter([map()]) -> [map()].
noise_test_filter(Tests0) ->
    Tests1 = [T || T = #{protocol_name := Name} <- Tests0, is_supported(Name)],
    Tests0Length = length(Tests0),
    case length(Tests1) < Tests0Length of
        true  ->
            ?debugFmt("WARNING: ~p test vectors out of ~p are unsupported",
                [Tests0Length - length(Tests1), Tests0Length]);
        false ->
            ok
    end,
    Tests1.
...
is_supported(Name) ->
    try
        _ = enoise_protocol:from_name(Name), true
    catch _:_ ->
        false
    end.
